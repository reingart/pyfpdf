...
def test_add_font_subset_complete(tmp_path):
    pdf = FPDF()
    pdf.alias_nb_pages(None)

    font_file_path = HERE / "Roboto-Regular.ttf"
    pdf.add_font("Roboto-Regular", fname=str(font_file_path), uni=True)

    pdf = FPDF()
    # re-using .pkl file generated by previous instance
    pdf.add_font("Roboto-Regular")
    pdf.set_font("Roboto-Regular", size=64)
    pdf.add_page()
    pdf.cell(txt="This is page {nb}")
    assert_pdf_equal(pdf, HERE / "add_font_subset_complete.pdf", tmp_path)
