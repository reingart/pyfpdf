...
from .py3k import PY3K, pickle, urlopen, BytesIO, Image, basestring, unicode, \
    exception, b, hashpath
...
class FPDF(object):
    ...
    def set_display_mode(self, zoom, layout='continuous'):
        ...
        if (
                            zoom == 'fullpage' or zoom == 'fullwidth' or zoom == 'real' or zoom == 'default' or not isinstance(
                zoom, basestring)):
            self.zoom_mode = zoom
        else:
            self.error('Incorrect zoom display mode: ' + zoom)
        if (
                        layout == 'single' or layout == 'continuous' or layout == 'two' or layout == 'default'):
    ...
    def set_draw_color(self, r, g=-1, b=-1):
        ...
        if ((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.draw_color = sprintf('%.3f %.3f %.3f RG', r / 255.0,
                                      g / 255.0, b / 255.0)
        if (self.page > 0):
    ...
    def set_fill_color(self, r, g=-1, b=-1):
        ...
        if ((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.fill_color = sprintf('%.3f %.3f %.3f rg', r / 255.0,
                                      g / 255.0, b / 255.0)
    ...
    def set_text_color(self, r, g=-1, b=-1):
        ...
        if ((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.text_color = sprintf('%.3f %.3f %.3f rg', r / 255.0,
                                      g / 255.0, b / 255.0)
        self.color_flag = (self.fill_color != self.text_color)
    ...
    def line(self, x1, y1, x2, y2):
        "Draw a line"
        self._out(sprintf('%.2f %.2f m %.2f %.2f l S', x1 * self.k,
                          (self.h - y1) * self.k, x2 * self.k,
                          (self.h - y2) * self.k))

    def _set_dash(self, dash_length=False, space_length=False):
        if (dash_length and space_length):
            s = sprintf('[%.3f %.3f] 0 d', dash_length * self.k,
                        space_length * self.k)
        else:
            s = '[] 0 d'
        self._out(s)
    ...
    def rect(self, x, y, w, h, style=''):
        ...
        if (style == 'F'):
            ...
            op = 'S'
        self._out(sprintf('%.2f %.2f %.2f %.2f re %s', x * self.k,
                          (self.h - y) * self.k, w * self.k, -h * self.k, op))
    ...
    def set_font(self, family, style='', size=0):
        ...
        if (
                    self.font_family == family and self.font_style == style and self.font_size_pt == size):
            return
        ...
        if fontkey not in self.fonts:
            ...
            if fontkey in self.core_fonts:
                if fontkey not in fpdf_charwidths:
                    ...
                    if fontkey not in fpdf_charwidths:
                        self.error(
                            'Could not include font metric file for' + fontkey)
                i = len(self.fonts) + 1
                self.fonts[fontkey] = {'i': i, 'type': 'core',
        ...
        if (self.page > 0):
            self._out(sprintf('BT /F%d %.2f Tf ET', self.current_font['i'],
                              self.font_size_pt))
    ...
    def link(self, x, y, w, h, link):
        ...
        self.page_links[self.page] += [(x * self.k, self.h_pt - y * self.k,
    ...
    def text(self, x, y, txt=''):
        ...
        s = sprintf('BT %.2f %.2f Td (%s) Tj ET', x * self.k,
    ...
    def rotate(self, angle, x=None, y=None):
        ...
        if angle != 0:
            ...
            cy = (self.h - y) * self.k
            s = sprintf(
                'q %.5F %.5F %.5F %.5F %.2F %.2F cm 1 0 0 1 %.2F %.2F cm', c,
                s, -s, c, cx, cy, -cx, -cy)
            self._out(s)
    ...
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=0, link=''):
        ...
        k = self.k
        if (
                        self.y + h > self.page_break_trigger and not self.in_footer and self.accept_page_break()):
            # Automatic page break
            x = self.x
            ws = self.ws
            if (ws > 0):
        ...
        if (fill == 1 or border == 1):
            if (fill == 1):
                ...
                op = 'S'
            s = sprintf('%.2f %.2f %.2f %.2f re %s ', self.x * k,
                        (self.h - self.y) * k, w * k, -h * k, op)
        if (isinstance(border, basestring)):
            x = self.x
            y = self.y
            if ('L' in border):
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', x * k,
                             (self.h - y) * k, x * k, (self.h - (y + h)) * k)
            if ('T' in border):
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', x * k,
                             (self.h - y) * k, (x + w) * k, (self.h - y) * k)
            if ('R' in border):
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', (x + w) * k,
                             (self.h - y) * k, (x + w) * k,
                             (self.h - (y + h)) * k)
            if ('B' in border):
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', x * k,
        ...
        if (txt != ''):
            ...
            if (self.underline):
                s += ' ' + self._dounderline(self.x + dx,
                                             self.y + .5 * h + .3 * self.font_size,
                                             txt)
            if (self.color_flag):
                s += ' Q'
            if (link):
                self.link(self.x + dx, self.y + .5 * h - .5 * self.font_size,
                          self.get_string_width(txt, True), self.font_size,
                          link)
        if (s):
            self._out(s)
        self.lasth = h
        ...
        while (i < nb):
            ...
            if (l > wmax):
                ...
                if (sep == -1):
                    ...
                    if (align == 'J'):
                        if ns > 1:
                            self.ws = (wmax - ls) / 1000.0 * self.font_size / (
                            ns - 1)
                        else:
                            self.ws = 0
                        if not split_only:
                    ...
                    if not split_only:
                        self.cell(w, h, substr(s, j, sep - j), b, 2, align,
                                  fill)
                    else:
    ...
    def write(self, h, txt='', link=''):
            ...
            if (l > wmax):
                ...
                if (sep == -1):
                    if (self.x > self.l_margin):
                        ...
                        w = self.w - self.r_margin - self.x
                        wmax = (
                               w - 2 * self.c_margin) * 1000.0 / self.font_size
                        i += 1
                        nl += 1
                        continue
                    if (i == j):
        ...
        # Last chunk
        if (i != j):
            self.cell(l / 1000.0 * self.font_size, h, substr(s, j), 0, 0, '',
                      0, link)

    @check_page
    def image(self, name, x=None, y=None, w=0, h=0, type='', link='',
              is_mask=False, mask_image=None):
        print("\nIM IN!!!!\n")
        ...
        if not name in self.images:
            ...
            if (type == 'jpg' or type == 'jpeg'):
                ...
                parsing_functions = [self._parsejpg, self._parsepng,
                                     self._parsegif]
        ...
        if not is_mask:
            self._out(
                sprintf('q %.2f 0 0 %.2f %.2f %.2f cm /I%d Do Q', w * self.k,
                        h * self.k, x * self.k, (self.h - (y + h)) * self.k,
                        info['i']))
        if (link):
    ...
    def _putfonts(self):
            ...
            if 'type' in info and info['type'] != 'TTF':
                ...
                if (not compressed and 'length2' in info):
                    ...
                    if (header and ord(font[info['length1']]) == 128):
                        ...
                        font = substr(font, 0, info['length1']) + substr(font,
                                                                         info[
                                                                             'length1'] + 6)
        ...
        for idx, k, font in flist:
            ...
            if (type == 'core'):
                ...
                for k in (
                'Ascent', 'Descent', 'CapHeight', 'Flags', 'FontBBox',
                'ItalicAngle', 'StemV', 'MissingWidth'):
                    s += ' /%s %s' % (k, font['desc'][k])
                filename = font['file']
                if (filename):
                    s += ' /FontFile'
    ...
    def _putTTfontwidths(self, font, maxUni):
        if font['unifilename']:
            cw127fname = os.path.splitext(font['unifilename'])[
                             0] + '.cw127.pkl'
        else:
            cw127fname = None
        ...
        for k, ws in sorted(range_.items()):
            cws = len(ws)
            if (k == nextk and not prevint and (
                not k in range_interval or cws < 3)):
    ...
    def _putimage(self, info):
        if 'data' in info:
            ...
            if (info['cs'] == 'Indexed'):
                self._out('/ColorSpace [/Indexed /DeviceRGB ' + str(
                    len(info['pal']) // 3 - 1) + ' ' + str(
                    self.n + 1) + ' 0 R]')
            else:
            ...
            if ('smask' in info):
                dp = '/Predictor 15 /Colors 1 /BitsPerComponent 8 /Columns ' + str(
                    info['w'])
                smask = {'w': info['w'], 'h': info['h'], 'cs': 'DeviceGray',
                         'bpc': 8, 'f': info['f'], 'dp': dp,
                         'data': info['smask']}
    ...
    def _putinfo(self):
        self._out('/Producer ' + self._textstring(
        ...
        self._out('/CreationDate ' + self._textstring(
    ...
    def _dounderline(self, x, y, txt):
        ...
        return sprintf('%.2f %.2f %.2f %.2f re f', x * self.k,
    ...
    def _parsejpg(self, filename):
        ...
        with f:
            ...
            f.seek(0)
            data = f.read()
        return {'w': width, 'h': height, 'cs': colspace, 'bpc': bpc,
    ...
    def _parsepng(self, filename):
        ...
        f.close()
        info = {'w': w, 'h': h, 'cs': colspace, 'bpc': bpc, 'f': 'FlateDecode',
                'dp': dp, 'pal': pal, 'trns': trns, }
        if (ct >= 4):
            ...
            if (ct == 4):
                ...
                for i in range(h):
                    ...
                    alpha += b(data[pos])
                    line = substr(data, pos + 1, length)
                    re_c = re.compile('(...).'.encode("ascii"),
                                      flags=re.DOTALL)
                    re_a = re.compile('...(.)'.encode("ascii"),
                                      flags=re.DOTALL)
                    color += re_c.sub(lambda m: m.group(1), line)
                    alpha += re_a.sub(lambda m: m.group(1), line)
            del data
